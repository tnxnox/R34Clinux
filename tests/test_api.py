...
from r34_client.api import Rule34Client
...
class Rule34ClientTests(unittest.TestCase):
    def test_search_posts_uses_auth_and_parses_json(self) -> None:
        ...
        with patch("r34_client.api.requests.get", return_value=FakeResponse(json.dumps(payload))) as get_mock:
    ...
    def test_autocomplete_tags_parses_label_and_value(self) -> None:
        ...
        with patch("r34_client.autocomplete.requests.get", return_value=FakeResponse(payload)):
            client = Rule34Client(user_id="1", api_key="secret")
