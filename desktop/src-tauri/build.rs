...
fn main() {
    ...
    if profile == "release" || !dist_dir.exists() || dist_dir.read_dir().map(|mut d| d.next().is_none()).unwrap_or(true) {
        ...
        match npm_install {
            ...
            Err(e) => {
                panic!("Failed to run npm install: {}. Is Node.js/npm installed?", e);
            }
        }
    }
