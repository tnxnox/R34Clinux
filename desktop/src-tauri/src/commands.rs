...
use crate::settings::{SettingsStore, AppSettings};
use crate::downloader::DownloadManager;
use crate::models::{Post, TagSuggestion, Friend, SyncStatus, SerializedPost};
...
#[tauri::command]
pub fn update_settings(state: tauri::State<'_, AppState>, payload: SettingsUpdatePayload) -> Result<Value, String> {
    ...
    let updated = AppSettings {
        ...
        download_directory: payload.download_directory.unwrap_or(current.download_directory),
        page_size: payload.page_size.unwrap_or(current.page_size),
        flaresolverr_enabled: payload.flaresolverr_enabled.unwrap_or(current.flaresolverr_enabled),
        flaresolverr_url: payload.flaresolverr_url.unwrap_or(current.flaresolverr_url),
        sync_conflict_strategy: payload.sync_conflict_strategy.unwrap_or(current.sync_conflict_strategy),
        background_sync_interval_minutes: payload.background_sync_interval_minutes.unwrap_or(current.background_sync_interval_minutes),
        download_naming_template: payload.download_naming_template.unwrap_or(current.download_naming_template),
        download_path_template: payload.download_path_template.unwrap_or(current.download_path_template),
        download_use_sample: payload.download_use_sample.unwrap_or(current.download_use_sample),
        download_sidecar_enabled: payload.download_sidecar_enabled.unwrap_or(current.download_sidecar_enabled),
        download_sidecar_format: payload.download_sidecar_format.unwrap_or(current.download_sidecar_format),
        download_max_retries: payload.download_max_retries.unwrap_or(current.download_max_retries),
    };
...
pub fn list_favorites(
    ...
    let posts = state.0.db.list_favorites(limit, collection.as_deref())
...
pub fn create_collection(
    ...
    let normalized = state.0.db.create_collection(&name).map_err(|e| e.to_string())?;
...
pub fn delete_collection(
    ...
    state.0.db.delete_collection(&name).map_err(|e| e.to_string())?;
    Ok(serde_json::json!({ "status": "ok" }))
}
...
pub fn assign_posts_to_collection(
    ...
    let count = state.0.db.assign_posts_to_collection(&postIds, &name).map_err(|e| e.to_string())?;
...
pub fn remove_posts_from_collection(
    ...
    let count = state.0.db.remove_posts_from_collection(&postIds, &name).map_err(|e| e.to_string())?;
...
pub fn add_friend(
    ...
    state.0.db.add_friend(&userId, &displayName, &notes.unwrap_or_default()).map_err(|e| e.to_string())?;
    Ok(serde_json::json!({ "status": "ok" }))
}
...
pub fn remove_friend(
    ...
    state.0.db.remove_friend(&userId).map_err(|e| e.to_string())?;
    Ok(serde_json::json!({ "status": "ok" }))
}
...
pub async fn download_post(
    ...
    match dest {
        Some(path) => Ok(serde_json::json!({ "status": "downloaded", "path": path.to_string_lossy() })),
...
pub fn start_sync(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        if lock_guard.is_err() {
            ...
            status.error = "Sync skipped: Another sync operation is already in progress.".to_string();
            return;
        }
...
pub async fn get_friend_favorites(
    ...
    let settings = state.0.settings.load();
    let flare_solver_url = if settings.flaresolverr_enabled { &settings.flaresolverr_url } else { "" };
