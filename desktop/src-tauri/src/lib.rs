        ...
        .setup(|app| {
            ...
            tauri::async_runtime::spawn(async move {
                loop {
                    ...
                    let run_res = if settings.has_credentials() && !settings.website_username.is_empty() && !settings.website_password.is_empty() {
                    ...
                    let sleep_secs = match run_res {
                        Ok(Some(wait_time)) => wait_time.max(1.0).min(60.0),
                        Ok(None) => 24.0 * 3600.0,
                        Err(e) => {
